...
def say(t, murl2file):
    ...
    return f'<span class="s"><b>@{esc(name)}</b> &middot; {dt}</span><br>{linkify(t.get("rawContent",""))}{media(t, murl2file)}'
...
def main():
    ...
    lines = ['<div class="r">Archivo de</div><h1>@__tinygrad__ — ' + str(len(tweets)) + ' tweets</h1><div class="r">Todo en orden cronológico, con los tweets a los que respondió.</div>']
    for t in tweets:
        pid = t.get('inReplyToTweetId') or t.get('inReplyToTweetIdStr')
        if pid is not None:
            pt = parents.get(str(pid)) or parents.get(pid)
            if pt:
                lines.append('<div class="r">Respondió a:</div><blockquote>' + say(pt, murl2file) + '</blockquote>')
        lines.append('<p>' + say(t, murl2file) + '</p>')
    html = '<!doctype html><html lang="es"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>@__tinygrad__ — archivo</title><style>' + CSS + '</style></head><body><main>' + '\n'.join(lines) + '</main></body></html>'
